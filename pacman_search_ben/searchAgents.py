...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner not in cornersVisited:
            undiscovered.append(corner)
    
    currentPoint = position
    for corner in undiscovered:
        closest, distance = findClosestPoint(currentPoint, undiscovered)
        currentPoint = closest
        heuristic += distance
        undiscovered.pop(undiscovered.index(currentPoint))
    return heuristic # Default to trivial solution

def findClosestPoint(point, points):
    if len(points) > 0:
        closestPoint = points[0]
        distance = util.manhattanDistance(points[0], point)
        for currentPoint in points:
            pointDistance = util.manhattanDistance(currentPoint, point)
...
def foodHeuristic(state, problem):
    ...
    return kruskal.lengthMST(foodlist + [position])
